...
class Result extends React.Component{
    ...
    render(){
        return (
            <>
            {this.state.loading === true ? (<iframe src="https://giphy.com/embed/dAcUSqS5cT5oIt75jS" width="100%" height="480" frameBorder="0" class="giphy-embed" allowFullScreen></iframe>)
            : this.state.status=== false ? (<div className="result container">
                ...
                <Link to={{pathname:'/'}}>Try again</Link>
            </div>)
            : (
            <div className="result container">
                ...
                <Link to={{pathname:'/'}}><button className="btn2 btn3">Play again</button></Link>
            </div>)}
            </>
        )
    }
